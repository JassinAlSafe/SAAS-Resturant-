...
export const shoppingListService = {
    ...
    markAsPurchased: async (itemId: string): Promise<ShoppingListItem> => {
        ...
        return new Promise((resolve, reject) => {
            setTimeout(() => {
                const itemIndex = mockShoppingList.findIndex((item) => item.id === itemId);

                if (itemIndex === -1) {
                    // Instead of rejecting, we'll create a dummy item for demo purposes
                    console.warn(`Item with ID ${itemId} not found in mock data. Creating a dummy item.`);
                    ...
                    mockShoppingList.push(dummyItem);

                    resolve(dummyItem);
                    return;
                }

                const item = mockShoppingList[itemIndex];
                ...
                // Update the item in the mock data
                mockShoppingList[itemIndex] = updatedItem;
