...
export default function ShoppingList() {
  ...
  const handleExportShoppingList = async () => {
    try {
      // Determine what data to export based on filter
      let dataToExport;

      if (selectedCategory === "all") {
        dataToExport = shoppingList;
      } else if (selectedCategory === "pending") {
        dataToExport = shoppingList.filter((item) => !item.isPurchased);
      } else if (selectedCategory === "completed") {
        dataToExport = shoppingList.filter((item) => item.isPurchased);
      } else if (selectedCategory === "generated") {
        dataToExport = shoppingList.filter((item) => item.isAutoGenerated);
      ...
      if (dataToExport.length === 0) {
        error(
          "Nothing to Export",
          "Your shopping list is empty. Add items before exporting."
        );
        return;
      }

      // Format and export the data
      const formattedData = formatShoppingListForExport(dataToExport);
      exportToExcel(formattedData, "Shopping_List", "Shopping List");

      // Show success notification
      success("Export Complete", "Shopping list has been exported to Excel.");
    } catch (error) {
