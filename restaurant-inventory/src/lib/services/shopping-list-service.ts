...
// Transform database response to ShoppingListItem
const mapDbToShoppingListItem = (data: DbShoppingListItem): ShoppingListItem => {
    return {
        id: data.id,
...
export const shoppingListService = {
    ...
    getItems: async (): Promise<ShoppingListItem[]> => {
        try {
            ...
            return data.map((item: DbShoppingListItem) => mapDbToShoppingListItem(item));
        } catch (error) {
                ...
                if (!existingItems || existingItems.length === 0) {
                    const { data: newItem, error: insertError } = await supabase
                        ...
                        .insert({
                            ...
                            unit: item.unit,
                            estimated_cost: (item.cost_per_unit || 0) * orderQuantity,
