  ...
  return (
    <div className="fixed bottom-4 right-4 z-50 w-96 max-w-[90vw] bg-white dark:bg-slate-900 p-4 rounded-lg shadow-lg border border-gray-200 dark:border-gray-800">
      ...
      <div className="space-y-2 text-sm max-h-[70vh] overflow-auto">
        ...
        <p><strong>Auth Context User:</strong> {user ? "Yes" : "No"}</p>
        <p><strong>Auth Context Session:</strong> {session ? "Yes" : "No"}</p>
        <p><strong>Auth Context Profile:</strong> {profile ? "Yes" : "No"}</p>
        <p><strong>Direct Supabase Session:</strong> {supabaseSession ? "Yes" : "No"}</p>
      ...
      <div className="mt-4 flex flex-wrap gap-2">
        <Button size="sm" onClick={refreshSession}>Refresh Data</Button>
        <Button size="sm" variant="outline" onClick={clearLocalStorage}>Clear Storage</Button>
        <Button size="sm" variant="secondary" onClick={forceRedirect}>Go to Dashboard</Button>
      </div>
    </div>
  );
}
