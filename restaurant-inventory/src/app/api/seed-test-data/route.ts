...
export async function POST() {
    try {
        ...
        const userId = 'ea330701-344b-4e8d-85b6-ce8d5f22534e';
        console.log('Seeding data for test user ID:', userId);

        // Step 1: Insert ingredients
        let { data: ingredientsData, error: ingredientsError } = await supabase
            .from('ingredients')
            .upsert(
        ...
        // Link ingredients to dishes (recipe ingredients)
        // This is simplified - in a real system you would create proper recipes
        ...
        console.error('Error in seed-test-data API route:', error);
        return NextResponse.json({
            success: false,
